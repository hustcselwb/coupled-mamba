...
def rmatvec(part_Us, part_VTs, x):
    ...
    return -x + torch.einsum('bd, bdi -> bi', xTU, part_VTs)    # (N, L'), but should really be (N, 1, (2d*L'))
...
def matvec(part_Us, part_VTs, x):
    ...
    return -x + torch.einsum('bid, bd -> bi', part_Us, VTx)     # (N, L'), but should really be (N, (2d*L'), 1)
...
def broyden(f, x0, threshold, eps=1e-3, stop_mode="rel", ls=False, name="unknown"):
    ...
    Us = torch.zeros(bsz, L, threshold).to(dev)     # One can also use an L-BFGS scheme to further reduce memory
    VTs = torch.zeros(bsz, threshold, L).to(dev)
    update = -matvec(Us[:,:,:nstep], VTs[:,:nstep], gx)      # Formally should be -torch.matmul(inv_jacobian (-I), gx)
    prot_break = False
    
    # To be used in protective breaks
    protect_thres = (1e6 if stop_mode == "abs" else 1e3)
    new_objective = 1e8
...
#         prev = (k%m) - 1 if (k%m) - 1 >= 0 else m - 1
#         X[:,k%m] = F[:,prev]
#         F[:,k%m] = f(X[:,k%m].reshape_as(x0)).reshape(bsz, -1)
        X = F.clone()
        F = f(X.reshape_as(x0)).reshape(bsz, -1)
        gx = (F - X).view_as(x0)
...
def anderson(f, x0, m=6, lam=1e-4, threshold=50, eps=1e-5, stop_mode='rel', beta=1.0, **kwargs):
#def anderson(f, x0, m=6, lam=1e-4, threshold=50, eps=1e-4, stop_mode='rel', beta=1.0, **kwargs):
    ...
    for k in range(2, threshold):
        ...
        H[:,1:n+1,1:n+1] = torch.bmm(G,G.transpose(1,2)) + lam*torch.eye(n, dtype=x0.dtype,device=x0.device)[None]
        # alpha = torch.solve(y[:,:n+1], H[:,:n+1,:n+1])[0][:, 1:n+1, 0]   # (bsz x n)
        alpha = torch.linalg.solve(H[:,:n+1,:n+1], y[:,:n+1])[0][:,1:n+1,0]   # (bsz x n)
        alpha = alpha.reshape(bsz, -1) 
        # X[:, k % m] = beta * (alpha @ F[:, :n].transpose(1, 2)) + (1 - beta) * (alpha @ X[:, :n].transpose(1, 2))
...
#         print(abs_diff, rel_diff)
        ...
        # Comment out this block of codes if tracking difference norm as it overwrites all solver steps upon convergence
        if trace_dict[stop_mode][-1] < eps:
            for _ in range(threshold-1-k):
                trace_dict[stop_mode].append(lowest_dict[stop_mode])
                trace_dict[alternative_mode].append(lowest_dict[alternative_mode])
