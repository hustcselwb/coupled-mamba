...
def train_epoch(data_list, label, model, optimizer):
    model.train()
    optimizer.zero_grad()
    loss, _, loss_dict,_ = model(data_list, label)
#     print('\r'+str([f'{k}: {v:.4f}' for k, v in loss_dict.items()]), end='')
...
def train(data_folder, modelpath, testonly):
    ...
    model = MMDynamic(dim_list, hidden_dim, num_class, dropout=0.5, jacobian_weight=100, f_thres=105, b_thres=106, use_deq=False)
    model.cuda()
#     if torch.cuda.device_count() > 1:
#         model = nn.DataParallel(model)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=step_size, gamma=0.5)
#     scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[50,300,400,600,1000,2000], gamma=0.5)
    if testonly:
        load_checkpoint(model, os.path.join(modelpath, data_folder, 'deq_50jacobian_rna+mirna.pt'))
        ...
        for epoch in range(num_epoch+1):
            ...
            if epoch % test_inverval == 0:
                ...
                if num_class == 2:
                    ...
                    print("Test F1 weighted: {:.5f}".format(f1_score(labels_trte[trte_idx["te"]], te_prob.argmax(1), average='weighted')))
                    print("Test F1 macro: {:.5f}".format(f1_score(labels_trte[trte_idx["te"]], te_prob.argmax(1), average='macro')))
        save_checkpoint(model.state_dict(), os.path.join(modelpath, data_folder), filename='checkpoint.pt')
