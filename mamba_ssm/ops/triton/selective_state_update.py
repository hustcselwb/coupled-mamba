...
def selective_state_update_ref(state, x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False):#状态更新
    """
    Argument:
        state: (batch, dim, dstate)
        x: (batch, dim)
    ...
    state.copy_(state * dA + dB * rearrange(x, "b d -> b d 1"))  # (batch, dim, dstate,更新状态
